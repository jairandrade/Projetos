...
//==================================================================================================//
//	Descrição: Este PE Está em todas as rotinas de alteração, inclusão, exclusão val. o acesso		//
//==================================================================================================//
User function MT410ACE()
    ...
    If !(__CUserId $ cParam)
        ...
        If !Empty(SC5->C5_XIDVNFK) .And. Alltrim(SC5->C5_XTIPONF) == '2' .And. nOpc == 1 //Excluir - VAlidacao da exclusao da NFSE
            cMsg 	:= "Pedido de serviço não pode ser excluído, você deve excluir o pedido de produto (ID-> "+SC5->C5_XIDVNFK+")!!!"
            lRet	:= .F.
        ElseIf !Empty(SC5->C5_XIDVNFK) .And. Alltrim(SC5->C5_XTIPONF) == '2' .And. !l410Auto .And. nOpc == 4 //Alteracao - Validacao da Alteracao da NFSE
            cMsg	:= "Pedido de serviço não pode ser alterado, você deve excluir o pedido de produto (ID-> "+SC5->C5_XIDVNFK+")!!!"
            lRet	:= .F.
        ElseIf !Empty(SC5->C5_XIDVNFK) .And. IsInCallStack("A410PCopia") //Copiar - Validacao da Alteracao da NFSE
            cMsg	:= "Pedido que gerou serviço não pode ser copiado!!!"
            lRet	:= .F.
        ...
        ElseIf !Empty(SC5->C5_XIDVNFK) .And. Alltrim(SC5->C5_XTIPONF) == '1' .And. !l410Auto .And. nOpc == 4 //Alteracao - Validacao da Alteracao da NFSE
            cMsg	:= "Pedido de serviço não pode ser alterado, pois já foi partido e gerou pedido de serviço. Caso tenha alteracoes, estorne o pedido e exclua o mesmo (ID-> "+SC5->C5_XIDVNFK+")!!!"
            lRet	:= .F.
    ...
    If cEmpAnt == '04' .and. lRet
        If (SC5->C5_XPVSPC == 'S') .And. (SC5->C5_XSTSSPP != '9') .And. nOpc == 1 //Excluir
            cMsg	:= "Este pedido não pode ser excluído, pois encontra-se controlado pela supplier, favor informar o setor responsável para envio do cancelamento, após o PV poderá ser cancelado"
            lRet	:= .F.
        EndIf
	EndIf
	...
	If AllTrim(SC5->C5_XSITLIB) == "9" .and. (nOpc == 1 .or. nOpc == 4 .or. (nOpc == 2 .and. IsInCallStack("MA410RESID")) ) .and. lRet  .and. lSepVld
		...
		If !lRet
			...
			cMsg := "Somente os usuários "+cUsu+" podem alterar um pedido em separação. KP_BLQPVSU"
		Endif		
	Endif
