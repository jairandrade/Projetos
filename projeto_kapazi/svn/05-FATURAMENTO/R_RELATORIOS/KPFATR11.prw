...
/**********************************************************************************************************************************/
/** user function KPFATR11()                 cStatus, "cPedido, cCli, cLj)                                                                                    **/
/**********************************************************************************************************************************/
user function KPFATR11(cPedVend,cpe) 
...
Private nposobsp   := aScan( aHeader, { |x| Alltrim(x[2])=="C6_X_OBSPR"  } )


	//Recebe o email do remetente
	//cMailVend := "pv@e-kapazi.com.br"
	cMailVend := GetNewPar('KP_CONTA','no-reply@kapazi.com.br')
	
	//Email do remetente
	cNomeFrom := "Pedido de Vendas - Kapazi"
	...
	//Layout  
	cWfTaskId := oWfProc:NewTask( "RELATORIOS", "\workflow\kapazi3.html" )     
  ...
  //popsiciona no cliente e loja
	...
	For nI:= 1 To Len (aCols)
		...
		If !aCols[nI,len(acols[nI])]
			...
			//incluido 28/07/2016 - MARCOS SULIVAN  
			cObsPr := aCols[nI][nposobsp] 
			...
			//IPI
			//REMOVIDO EM 29/112017
			// INSERIDO CALCULO MAIS ABAIXO (u_Calcimp)
			...
			//incluido 28/07/2016 - MARCOS SULIVAN    
			Aadd(oWfHtml:ValByName("IT.OBSPR"),			ALLTRIM(aCols[nI][nposobsp]))
			
			//Aadd(oWfHtml:ValByName("IT.DTENTR"),		DTOC(aCols[nI][nposentr]))	 SC6->C6_X_OBSPR
	  EndIf
	
	Next
	//incluido em 29/11/2017
	aImp	:=	U_CalcImp(cPedido)
	nTotIPI := aImp[2]
	//Ate aqui - em 29/11/2017
  ...
  oWfProc:cFromAddr := cMailVend
  oWfProc:cFromName := cNomeFrom  
  oWfProc:cTo       := ALLTRIM(SA1->A1_EMAIL) +";"+ ALLTRIM(cMailRep) //"luis@rsacsolucoes.com.br"//
  //oWfProc:cCC       := "pv@e-kapazi.com.br"
  oWfProc:cCC       := cMailVend
  oWfProc:cSubject  := "Pedido de Venda nº " + Alltrim(cPedido)
  oWfProc:bReturn   := Nil 
