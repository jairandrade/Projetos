...
#Include "topconn.ch"
//==================================================================================================//
//	Programa: KP97PE05		|	Autor: Luis Paulo							|	Data: 07/09ñ/2018	//
//==================================================================================================//
//	Descrição: Funcao responsavel por gerar o arquivo Supplier										//
//																									//
//==================================================================================================//
User Function KP97PE05()
Private cNmArq	:= ""
Private nHdlLog	:= ""
Private cPData		:= GetSrvProfString ("ROOTPATH","")
Private cDirtAtu	:= "\Supplier\Ren Pre Aut\"
Private lCriou		:= .F.
Private cArqAtu		:= ""
Private cArqNew		:= ""
Private cDir		:= ""
...
Private cDirProc	:= "\Supplier\Ren Pre Aut"
Private cIdSP		:= ""	
Private lErroE		:= .F.

If ValItenS() //Valida se tem itens pendentes de verificacao
		U_KP97A12() //Processa os registro pendentes da supplier
		MsgInfo("Processo finalizado!!","KAPAZI - PEDIDOS DE VENDA SUPPLIER CARD")
	Else
		MsgInfo("Existem itens com inconsistencias(pre aut), favor verificar!!","KAPAZI - PEDIDOS DE VENDA SUPPLIER CARD")
...
cSql	:= " UPDATE "+ RetSqlName("ZS5") +" "
If lRet
		cSql	+= " SET ZS5_NMARQI = '"+cNmArq+"'"//, ZS5_ENVEMA = 'S' "
	Else
		cSql	+= " SET ZS5_NMARQI = '"+cNmArq+"'" //, ZS5_ENVEMA = 'N' "
EndIf
cSql	+= " WHERE ZS5_XIDINT = '"+cIdSP+"' "
