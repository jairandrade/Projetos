...
If cAliasS1->(EOF())
	...
	Else
		...
		If FM_Direct( cDirProc, .F., .F. )	//Caso nao tenha o diretorio Supplier, criaa
			...
			lCriou	:= .T.
		EndIf
		cIdSP	:= GetSx8Num("ZS5", "ZS5_XIDINT","ZS5_XIDINT" + "\system\"+RetSqlName("ZS5"),1)
		ConfirmSx8()
		xGeraXml()
EndIf
...
While !cAliasS1->(EOF()) .And. lCriou
	...
	cLinha	:= 	cAliasS1->ZS5_CODSOL+";"+;
				Alltrim(cAliasS1->ZS5_CGC)	+";"+;
				Alltrim(cAliasS1->ZS5_CGCK)	+";"+;
				Alltrim(cAliasS1->ZS5_TIPOTR)+";"+;
				Alltrim(cAliasS1->ZS5_CODPVS)+";"+;
				Alltrim(Transform(cAliasS1->ZS5_VALPV,"@E 999,999,999.99"))+";"+;
				Alltrim(cAliasS1->ZS5_QTDPAR)+";"+;
				Alltrim(cAliasS1->ZS5_DPRIVE)+";"+;
				Alltrim(cAliasS1->ZS5_CONDSP)+";"+;
				Alltrim(cAliasS1->ZS5_QTDDEP)+";"+;
				Alltrim(cAliasS1->ZS5_TXCLI)+";"+;
				Alltrim(cAliasS1->ZS5_TXAPAR)+";"+;
				Alltrim(cAliasS1->ZS5_TXPPAR)+";"+;
				Alltrim(cAliasS1->ZS5_FORREC)+";"+;
				Alltrim(cAliasS1->ZS5_PRECPA)+";"+;
				Alltrim(cAliasS1->ZS5_OBS)+"" + cQuebra
	...
	ZS5->(DbSetOrder(1))
	ZS5->(DbGoTop())
	ZS5->(DbGoTo(cAliasS1->RECORECO))
	RecLock("ZS5",.F.)
	ZS5->ZS5_XIDINT	:= cIdSP
	ZS5->(MsUnlock())
...
cNmArq := "REN-PRE-AUT-PV-SUPP" + DTOS(DATE()) + SUBSTR(TIME(),1,2) + SUBSTR(TIME(),4,2) + "-" +cEmpAnt+cFilAnt+".xml"
...
While !cAliasS1->(EOF())
	...
	cValor		:= Alltrim(Transform(cAliasS1->ZS5_VLRPAU,"@E 999,999,999.99"))
	oExcel:AddRow("SUPPLIER CARD","PEDIDOS"	,{	Alltrim(cAliasS1->ZS5_CODSO),;
												Alltrim(cAliasS1->ZS5_CGC),;
												Alltrim(cAliasS1->ZS5_TIPOTR),;
												Alltrim(cAliasS1->ZS5_CODPV),;
												cValor})
