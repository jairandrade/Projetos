...
User Function SXBSG1A()
Local lRet       := .F.
Local nRetorno   := 0
Local aSearch    := {"G1_COD","B1_DESC"}
Local cQuery     := ""

cQuery := " SELECT DISTINCT TOP 10000 (G1_COD) G1_COD,B1_DESC AS B1_DESC,(SELECT TOP 1 R_E_C_N_O_ FROM SG1040  WHERE D_E_L_E_T_ = '' AND G1_COD = SG1.G1_COD)  AS RECNO 
cQuery += " FROM "+RetSqlName("SG1")+" SG1 " 
cQuery += " INNER JOIN SB1010 SB1 ON SG1.G1_COD = SB1.B1_COD AND SB1.D_E_L_E_T_ = '' AND SB1.B1_MSBLQL <> '1' 
cQuery += " WHERE SG1.D_E_L_E_T_ = '' AND SG1.G1_FIM >= '"+DTOS(Date())+"' "

If F3SG1PASS( cQuery, "TMPG1A", "RECNO"    , @nRetorno,, aSearch, "SG1" )
     SG1->(dbGoto(nRetorno))
     lRet := .T.
EndIf
