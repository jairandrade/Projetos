			...
			If !IsBlind()
				If !(M->C5_VEND1 $ cVnd1C)
					If !"TEST" $ GetEnvServer()
						lRet	:= .F.
					Endif
					MsgStop("Não é possivel confirmar a inclusão deste pedido, verifique o Cliente ou o Vendedor.","Cliente ou Vendedor inválido para o usuário!")
				EndIf
			EndIf
			(cAlias3)->(DbCloseArea())
		EndIf
	...
	For nX := 1 to Len(aCols)
		If !Atail(aCols[nX])
			...
			If SB1->( MsSeek( xFilial("SB1")+cProd ) )
				If (!Empty(SB1->B1_SEGUM) .and. AllTrim(SB1->B1_UM) == "M2" ) .or. SB1->B1_XUSGUM == 'S'					
					aCols[nX][nPTp] := "2"
				Else
					aCols[nX][nPTp] := "1"
				Endif
