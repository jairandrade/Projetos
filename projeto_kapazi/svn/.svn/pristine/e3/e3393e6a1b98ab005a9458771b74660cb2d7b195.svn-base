...
#Include "tbiconn.ch"
//==================================================================================================//
//	Programa: GerNFSv		|	Autor: Luis Paulo							|	Data: 22/07/2019	//
//==================================================================================================//
//	Descrição: Funcao para gerar NF de Serviço														//
//																									//
//==================================================================================================//
User Function GNFSPEN()
local cQry		:= ""
local nRegs		:= 0
local nCount	:= 0
local cIdNFSE	:= ""
...
cQr := " SELECT C5_EMISSAO EMISSAO,C5_NOTA,R_E_C_N_O_ AS RECOPV,*
cQr += " FROM SC5040
cQr += " WHERE D_E_L_E_T_ = ''
cQr += " AND C5_XIDVNFK <> ''
cQr += " AND C5_NOTA = ''
cQr += " AND C5_EMISSAO >= '20190613'
cQr += " ORDER BY C5_EMISSAO

TcQuery cQr new alias (cAlias)
Count To nRegs
...
While  !((cAlias)->(EOF()))
	...
	DbSelectArea("SC5")
	SC5->(DbGoTop())
	SC5->(DbGoTo((cAlias)->RECOPV))
	
	cIdNFSE	 := SC5->C5_XIDVNFK
	cTxtC5kp := SC5->C5_MSGCLI
...
//	MV_PAR01	:= 2
	...
	If lMsErroAuto
			DisarmTransaction()
			MostraErro()
			Alert("Erro na geracao de faturas!")
		Else
