...
Static Function DANFEProc(	oDanfe	, lEnd		, cIdEnt	, cVal1,; 
		...
		(cAliasSF3)->F3_NFISCAL <= MV_PAR02
			...
			If  Empty((cAliasSF3)->F3_DTCANC) .And. &cWhere //.And. AModNot((cAliasSF3)->F3_ESPECIE)=="55"
				...
				If (SubStr((cAliasSF3)->F3_CFO,1,1)>="5" .Or. (cAliasSF3)->F3_FORMUL=="S") .And. aScan(aNotas,{|x| x[4]+x[5]+x[6]+x[7]==(cAliasSF3)->F3_SERIE+(cAliasSF3)->F3_NFISCAL+(cAliasSF3)->F3_CLIEFOR+(cAliasSF3)->F3_LOJA})==0
					...
					//<customizado>
					//adicionado por Antonio
					//Para filtrar notas emitidas pelo usuário
					...
					If MV_PAR04==2
						If type("_lFiltraNF") <> "U" .and. _lFiltraNF					
							...
							IF SF2->( FieldPos("F2_K_USRCO") ) > 0
								cQry := "select * from "+RetSQLName("SF2")+ " where D_E_L_E_T_ <> '*' and F2_FILIAL = '"+xFilial("SF2")+"' and F2_SERIE = '"+(cAliasSF3)->F3_SERIE+"' and F2_DOC = '"+(cAliasSF3)->F3_NFISCAL+"'  "
				...
				For nX := 1 To nLenNotas
					If !Empty(aXML[nX][2])
						...
						If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
							...
							if type("oRetNF") == "U"
								cXml := u_removeCaracEspecial( aXML[nX][2] ) 
								oRetNF := XmlParser(cXml ,"_",@cAviso,@cErro)
							endif

							if type("oRetNF:_NFEPROC") <> "U"
	...
	If Len(aGrvSF3)>0 .And. SF3->(FieldPos("F3_CHVNFE"))>0
		...
		For nI := 1 To Len(aGrvSF3)
			...
			If SF3->(MsSeek(xFilial("SF3")+aGrvSF3[nI,1]+aGrvSF3[nI,2]+aGrvSF3[nI,3]+aGrvSF3[nI,4]+aGrvSF3[nI,5])) .And. lChave;
...
/*/
//-----------------------------------------------------------------------
user function removeCaracEspecial( cTexto ) 
...
user function listHTMLCaracter()
	...
	aAdd( aCaracter,{'İ','&Yacute;'	})
	aAdd( aCaracter,{'Ş','&THORN;'	})
