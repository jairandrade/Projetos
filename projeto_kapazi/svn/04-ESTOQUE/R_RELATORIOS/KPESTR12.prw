...
User Function KPESTR12()
	
	Private cPerg   	:= "KPESTR12"				//Grupo de pergunta
...
Static Function RelEntLv()
	...
	//Cria o relatório
	oReport := TReport():New(cPerg, "ENTRADA X NCM", Nil, {|oReport| ImpRel() }, "Este relatório exibe as entradas X NCM")
...
Static Function QryBd2()
  ...
  Local cQr 		:= ""             //Recebe a query
	...
	//Monta a query dos itens 
		...
		cQr += "		SD1.D1_DOC		DOCUMENTO,
		cQr += "		SD1.D1_VALICM	VALICM,
		cQr += "		SD1.D1_VALIPI	VALIPI,
		cQr += "		SD1.D1_CF		CODFIS,
		cQr += "		SD1.D1_TOTAL	VALOR,
		cQr += "		SD1.D1_EMISSAO	EMISSAO


		cQr += "		FROM	"+ RetSqlName("SB1") +"		SB1

		cQr += "		JOIN	"+ RetSqlName("SD1") +"		SD1
		cQr += "  	ON	SD1.D1_COD = SB1.B1_COD
		cQr += "	AND SD1.D_E_L_E_T_ = ''
		cQr += "	AND SD1.D1_EMISSAO BETWEEN  '" + Dtos(mv_par01) + "' AND '" + Dtos(mv_par02) + "'
