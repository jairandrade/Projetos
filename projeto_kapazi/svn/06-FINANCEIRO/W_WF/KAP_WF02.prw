...
+ ---------------------------------------------------------------------------------------------------------------------------------------+
| Financerio                                                                                                                             |
| Schedule - Aviso de boletos vencidos a mais de 10 dias OU conforme parametro KP_DAVISO2                                                    |
...
user Function KAP_WF02(cParEmp)
	...
	cQry += "    AND floor(convert(float, GetDate() - convert(datetime, E1_VENCREA)))  = (" + GetMv("KP_DAVISO2") + ")
...
static function fSendAvs()
	...
	cVenc := Substr(TRB->E1_VENCREA,7,2)+"/"+Substr(TRB->E1_VENCREA,5,2)+"/"+Substr(TRB->E1_VENCREA,1,4) //ctod(TRB->E1_VENCREA)//Left(cVenc, Len(cVenc) - 2)


	//Andre-Rsac 23.01.2017
	cDiasVEN := GetMv("KP_DAVISO2")
	...
	//Layout
	cWfTaskId := oWfProc:NewTask( "RELATORIOS",  "\workflow\Aviso de Vencimento2.html")
