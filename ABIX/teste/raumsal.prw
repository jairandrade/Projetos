...
±±³Program.       ³ Data   ³ FNC/CH        ³ Motivo da Alteracao          ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³ Raquel Hager  ³04/09/14³TQMIEK         ³ Funcoes como User_Function   ³±± 
...
While   !EOF() .And. &cInicio <= cFim
    ...
    AADD(aFixo,{SRA->RA_FILIAL,SRA->RA_MAT,If(lOfuscaNom,Replicate('*',len(SRA->RA_NOME)),SRA->RA_NOME),SRA->RA_CC,SRA->RA_Salario,;
...
cDET	+=	fAnoMes(aFixo[nJ,6],Date())+SPACE(9)+TRANSFORM(aFixo[nJ,5],"@E 999,999,999.99") 
Impr(cDet,"C")
...
dbSelectArea("SX5")
cChave:=If(Empty(xFilial('SX5')), Space(FWGETTAMFILIAL), cFilial)+'41'
