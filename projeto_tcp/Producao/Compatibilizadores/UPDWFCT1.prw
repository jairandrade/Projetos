...
/*/
//--------------------------------------------------------------------
User Function UPDWFCT1( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPDWFCT1" )
				Else
					MsgStop( "Atualização não Realizada.", "UPDWFCT1" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'CT2'																	, .T. }, ; //X3_ARQUIVO
	{ '37'																	, .T. }, ; //X3_ORDEM
	{ 'CT2_TPSALD'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Tipo do Sld'															, .T. }, ; //X3_TITULO
	{ 'Tipo de Sld'															, .T. }, ; //X3_TITSPA
	{ 'Balance Type'														, .T. }, ; //X3_TITENG
	{ 'Tipo do Saldo?'														, .T. }, ; //X3_DESCRIC
	{ '+Tipo de Saldo          ?'											, .T. }, ; //X3_DESCSPA
	{ 'Balance Type?'														, .T. }, ; //X3_DESCENG
	{ '!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	...
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ ''																	, .T. }, ; //X3_VLDUSER
	{ '1=Real;2=Orcado;3=Gerencial;4=Pre-Lancamento'						, .T. }, ; //X3_CBOX
	{ '1=Real;2=Presupuestado;3=Gestion;4=Preasiento'						, .T. }, ; //X3_CBOXSPA
	{ '1=Real;2=Budgeted;3=Managerial;4=Pre-Entry'							, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
	{ ''																	, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
...
aAdd( aSX6, { ;
	...
	'TCP_GRAPLC'															, ; //X6_VAR
	'C'																		, ; //X6_TIPO
	'Define grupo alçada para receber WF Aprovação'							, ; //X6_DESCRIC
	'Define grupo alçada para receber WF Aprovação'							, ; //X6_DSCSPA
	'Define grupo alçada para receber WF Aprovação'							, ; //X6_DSCENG
	'Lançamento Contábeis Manual CTBA102'									, ; //X6_DESC1
	'Lançamento Contábeis Manual CTBA102'									, ; //X6_DSCSPA1
	'Lançamento Contábeis Manual CTBA102'									, ; //X6_DSCENG1
	''																		, ; //X6_DESC2
	''																		, ; //X6_DSCSPA2
	''																		, ; //X6_DSCENG2
	'999990'																, ; //X6_CONTEUD
...
aAdd( aSX6, { ;
	...
	'TCP_MAILLC'															, ; //X6_VAR
	'C'																		, ; //X6_TIPO
	'Informe o e-mail para receber notificação Workflow'					, ; //X6_DESCRIC
	'Informe o e-mail para receber notificação Workflow'					, ; //X6_DSCSPA
	'Informe o e-mail para receber notificação Workflow'					, ; //X6_DSCENG
	'de aprovação pré lançamento contábil CTBA102'							, ; //X6_DESC1
	'de aprovação pré lançamento contábil CTBA102'							, ; //X6_DSCSPA1
	'de aprovação pré lançamento contábil CTBA102'							, ; //X6_DSCENG1
	''																		, ; //X6_DESC2
	''																		, ; //X6_DSCSPA2
	''																		, ; //X6_DSCENG2
	'willian.kaneta@almasge.com.br'											, ; //X6_CONTEUD
	'willian.kaneta@almasge.com.br'											, ; //X6_CONTSPA
	'willian.kaneta@almasge.com.br'											, ; //X6_CONTENG
	'U'																		, ; //X6_PROPRI
	''																		, ; //X6_VALID
	''																		, ; //X6_INIT
	''																		, ; //X6_DEFPOR
...
aAdd( aSX6, { ;
	...
	'TCP_WFLCTO'															, ; //X6_VAR
	'L'																		, ; //X6_TIPO
	'Habilita envio WorkFlow Aprovação Lançamentos'							, ; //X6_DESCRIC
	'Habilita envio WorkFlow Aprovação Lançamentos'							, ; //X6_DSCSPA
	'Habilita envio WorkFlow Aprovação Lançamentos'							, ; //X6_DSCENG
	'Contábeis incluídos manualmente rotina CTBA102'						, ; //X6_DESC1
	'Contábeis incluídos manualmente rotina CTBA102'						, ; //X6_DSCSPA1
	'Contábeis incluídos manualmente rotina CTBA102'						, ; //X6_DSCENG1
	''																		, ; //X6_DESC2
	''																		, ; //X6_DSCSPA2
	''																		, ; //X6_DSCENG2
	'.T.'																	, ; //X6_CONTEUD
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPDWFCT1" ) ) ) ;
