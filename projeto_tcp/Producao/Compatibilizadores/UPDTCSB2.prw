...
/*/
//--------------------------------------------------------------------
User Function UPDTCSB2( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPDTCSB2" )
				Else
					MsgStop( "Atualização não Realizada.", "UPDTCSB2" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
If ( lOpen := MyOpenSm0(.T.) )
	...
	If lOpen
		...
		For nI := 1 To Len( aRecnoSM0 )
			...
			//Executa Procedure atualizada do Order point
			If cEmpAnt == "02"
				
				cAno := Substr( DTOS(SuperGetMV("MV_ULMES",.F.,CTOD("30/06/2020"))), 1, 4)
				cMes := Substr( DTOS(SuperGetMV("MV_ULMES",.F.,CTOD("30/06/2020"))), 5, 2)
				cData := cAno + cMes  
				...
				IF Empty(TcSqlError())
					cTmpExec := ElapTime( cHraIni, cHraFim )
					MsgInfo("Procedure de geração de dados para ORDER POINT executada com sucesso. Tempo para execução: " + cTmpExec)
				Else
...
aAdd( aSX3, { ;
	...
	{ 2																		, .T. }, ; //X3_DECIMAL
	{ 'Sld.Atu.'															, .T. }, ; //X3_TITULO
	{ 'Sld. Actual'															, .T. }, ; //X3_TITSPA
	{ 'Current Bal.'														, .T. }, ; //X3_TITENG
	{ 'Saldo atual em valor'												, .T. }, ; //X3_DESCRIC
	{ 'Saldo Actual en Valor'												, .T. }, ; //X3_DESCSPA
	{ 'Value Current Balance'												, .T. }, ; //X3_DESCENG
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPDTCSB2" ) ) ) ;
