...
/*/
//--------------------------------------------------------------------
User Function UPDPC001( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPDPC001" )
				Else
					MsgStop( "Atualização não Realizada.", "UPDPC001" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ '@E 999,999,999.999999'												, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'SD1'																	, .T. }, ; //X3_ARQUIVO
	{ 'B2'																	, .T. }, ; //X3_ORDEM
	{ 'D1_QTDPEDI'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 16																	, .T. }, ; //X3_TAMANHO
	{ 6																		, .T. }, ; //X3_DECIMAL
	{ 'Qtde Pedido'															, .T. }, ; //X3_TITULO
	{ 'Ctd.Pedido'															, .T. }, ; //X3_TITSPA
	{ 'Qty Ordered'															, .T. }, ; //X3_TITENG
	{ 'Qtde em Pedido de Compras'											, .T. }, ; //X3_DESCRIC
	{ 'Ctd.en Pedido de Compras'											, .T. }, ; //X3_DESCSPA
	{ 'Purchase Order Quantity'												, .T. }, ; //X3_DESCENG
	{ '@E 999,999,999.999999'												, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
/*aAdd( aSX3, { ;
	...
	{ 'Pror. %'																, .T. }, ; //X3_TITENG
	{ '% Rateio do Item'													, .T. }, ; //X3_DESCRIC
	{ '% Prorrateo del Item'												, .T. }, ; //X3_DESCSPA
	{ 'Item Proration %'													, .T. }, ; //X3_DESCENG
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPDPC001" ) ) ) ;
