...
cSql := " SELECT PC_FILIAL, PC_MAT, PC_DATA, SUM(PC_QUANTC) PC_QUANTC,  PC_CC, RA_NOME  FROM " + RetSqlName("SPC") + " SPC  "
cSql += " INNER JOIN " + RetSqlName("SRA") + " SRA ON RA_FILIAL = '" + xFilial("SRA") + "' 	"
cSql += " AND RA_MAT = PC_MAT AND SRA.D_E_L_E_T_ = ' ' 										"
cSql += " WHERE 																			"
cSql += " PC_FILIAL BETWEEN '" + MV_PAR01 + "' AND '" + MV_PAR02 + "'						"
cSql += " AND PC_MAT BETWEEN '" + MV_PAR03 + "' AND '" + MV_PAR04 + "' 						"
cSql += " AND PC_CC BETWEEN '" + MV_PAR05 + "' AND '" + MV_PAR06 + "' 						" 
cSql += " AND PC_DATA BETWEEN '" + DTOS(MV_PAR07) + "' AND '" + DTOS(MV_PAR08) + "' 		"
cSql += " AND LTRIM(RTRIM(PC_PD)) IN ('106','107','109','113','210') 						"
cSql += " AND SPC.D_E_L_E_T_ = ' ' 															"
cSql += " GROUP BY PC_FILIAL, PC_MAT, PC_DATA, PC_CC, RA_NOME 								"
...
// Primeira seção
oSection1:= TRSection():New(oReport,cTitle,{"SPC"})
...
TRCell():New(oSection1,"PC_DATA"		, "SPC" , "Data")
TRCell():New(oSection1,"PC_MAT"			, "SPC" , "Matrícula")
TRCell():New(oSection1,"PC_CC"			, "SPC" , "C.Custo")
TRCell():New(oSection1,"RA_NOME"		, "SRA" , "Nome")
TRCell():New(oSection1,"PC_QUANTC"		, "SPC", "Hora Extra"	)
...
For nX := 1 To Len(_aRet)
	oSection1b:Cell("PC_DATA"):SetBlock({|| _aRet[nX][1]}) 
	oSection1b:Cell("PC_MAT"):SetBlock({|| _aRet[nX][2]}) 
	oSection1b:Cell("PC_CC"):SetBlock({|| _aRet[nX][3]}) 
	oSection1b:Cell("RA_NOME"):SetBlock({|| _aRet[nX][4]}) 
	oSection1b:Cell("PC_QUANTC"):SetBlock({|| _aRet[nX][5]}) 
