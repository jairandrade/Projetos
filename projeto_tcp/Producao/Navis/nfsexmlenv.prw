...
user function nfseXMLEnv( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela,aAIDF )
	...
	Local lIntTur 		:= SuperGetMV("MV_INTTUR",,.F.)	
	//Grupo de clientes com exceção fiscal devido a lei
	...
	private nCountSF3	:= 0
	private lHasSigaMatX:= Type("oSigamatX") == "U"
	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
           ...
           If cMvMsgTrib $ "1-3"
			...
			//14.07.2016 - Luiz F. Inclusão de campos customizados para mensagem da NF.
			If SD2->(FieldPos("D2_YMSGNF"))<>0
			   cField  +=",D2_YMSGNF"				    
			EndIf			
			If SD2->(FieldPos("D2_YMSGNF1"))<>0
			   cField  +=",D2_YMSGNF1"				    
			EndIf

			If SD2->(FieldPos("D2_YMSGNF2"))<>0
			   cField  +=",D2_YMSGNF2"				    
			EndIf
			If SD2->(FieldPos("D2_YMSGNF3"))<>0
			   cField  +=",D2_YMSGNF3"				    
			EndIf
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If IntTms()
					...
					Else
						...
						Else
							If Alltrim(if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN )) == "5208707" //Goiania
								cMunPrest := Alltrim(aDest[25])
								cDescMunP := aDest[08] 
							Else
								If ((cAliasSD2)->D2_ORIGLAN $ "LO")
									cMunPrest := if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN )
				...
				Else
					...
					ElseIf Alltrim(if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN )) == "3507605" .And. SF4->F4_ISSST == '3'			// Bragança Paulista
						cMunPrest := Alltrim(if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ))
						cDescMunP := Alltrim(if( lHasSigaMatX,SM0->M0_CIDCOB,oSigamatX:M0_CIDCOB ))
					Else
						If Alltrim(if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN )) == "5208707" //Goiania
							cMunPrest := Alltrim(aDest[25])
							cDescMunP := aDest[08] 
						Else
							...
							If ((cAliasSD2)->D2_ORIGLAN $ "LO")
								cMunPrest := if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN )
								cDescMunP := Alltrim(if( lHasSigaMatX,SM0->M0_CIDENT,oSigamatX:M0_CIDENT ))
							Elseif ((cAliasSD2)->D2_ORIGLAN $ "VD")
								cMunPrest := aDest[07]
								If Empty(cMunPrest)
									cMunPrest := if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN )
					...
					If lSC5 .And. SC5->(FieldPos("C5_MUNPRES")) > 0 .And. Empty(SC5->C5_MUNPRES) .And. if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) == "3509502"
				...
				If !Empty((cAliasSD2)->D2_YMSGNF) .Or. !Empty((cAliasSD2)->D2_YMSGNF1) .Or. !Empty((cAliasSD2)->D2_YMSGNF2) .Or. !Empty((cAliasSD2)->D2_YMSGNF3)
				   If nCont == 1
				   	cNatOper := ""
				   EndIf
					cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim((cAliasSD2)->D2_YMSGNF)),AllTrim((cAliasSD2)->D2_YMSGNF))
					cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim((cAliasSD2)->D2_YMSGNF1)),AllTrim((cAliasSD2)->D2_YMSGNF1))
					cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim((cAliasSD2)->D2_YMSGNF2)),AllTrim((cAliasSD2)->D2_YMSGNF2))
					cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim((cAliasSD2)->D2_YMSGNF3)),AllTrim((cAliasSD2)->D2_YMSGNF3))
				EndIf
				...
				If !Empty((cAliasSD2)->D2_NFORI) 
					If (cAliasSD2)->D2_TIPO $ "DBN"
						...
						If DbSeek(xFilial("SD1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
							...
							aadd(aNfVinc,{SD1->D1_EMISSAO,SD1->D1_SERIE,SD1->D1_DOC,IIF(SD1->D1_TIPO $ "DB",IIF(SD1->D1_FORMUL=="S",if( lHasSigaMatX,SM0->M0_CGC,oSigamatX:M0_CGC ),SA1->A1_CGC),IIF(SD1->D1_FORMUL=="S",if( lHasSigaMatX,SM0->M0_CGC,oSigamatX:M0_CGC ),SA2->A2_CGC)),if( lHasSigaMatX,SM0->M0_ESTCOB,oSigamatX:M0_ESTCOB ),SF1->F1_ESPECIE})
						EndIf
					Else
						aOldReg  := SD2->(GetArea())
						...
						If DbSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
							...
							aadd(aNfVinc,{SF2->F2_EMISSAO,SD2->D2_SERIE,SD2->D2_DOC,if( lHasSigaMatX,SM0->M0_CGC,oSigamatX:M0_CGC ),if( lHasSigaMatX,SM0->M0_ESTCOB,oSigamatX:M0_ESTCOB ),SF2->F2_ESPECIE})
						EndIf
						RestArea(aOldReg)
						RestArea(aOldReg2)
					EndIf
				...
				While !Eof() .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
					...
					dbSkip()
				EndDo
				If if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) == "4205407" //florianopolis
					nValTotPrd := IIF(!(cAliasSD2)->D2_TIPO$"IP",IIF(if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) == "3550308",(cAliasSD2)->D2_PRCVEN * (cAliasSD2)->D2_QUANT,(cAliasSD2)->D2_TOTAL),0)
				Else
					nValTotPrd := IIF(!(cAliasSD2)->D2_TIPO$"IP",IIF(if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) == "3550308",(cAliasSD2)->D2_PRCVEN * (cAliasSD2)->D2_QUANT,(cAliasSD2)->D2_TOTAL),0)+((cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR)
				EndIf	
				If lAglutina
					If Len(aProd) > 0			
						...
						If nX > 0						
							...
							If if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) == "3550308"									
				...
				If !lAglutina .Or. Len(aProd) == 0
					...
					aadd(aProd,	{Len(aProd)+1,;
								...
								IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_PRCVEN+IIf(if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) == "4205407",0,(((cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR)/(cAliasSD2)->D2_QUANT)),0),;								
								...
								(cAliasSD2)->D2_VALFRE,;
								a410Arred( IIF(!(cAliasSD2)->D2_TIPO$"IP",IIF(if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) == "3550308",(cAliasSD2)->D2_PRCVEN * (cAliasSD2)->D2_QUANT,(cAliasSD2)->D2_TOTAL),0), FuCamArren(nCamPrcv,nCamQuan,nCamTot) ),; // Valor Liquido
		...
		if !empty(aNota)
			if len (aProd) > 0
				...
				cString	+= assina( aDeduz, aNota, aProd, aTotal, aDest, aDeducao )
				...
				cString	+= servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe, aCST, aDest[22], if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ), cF4Agreg ,nDescon,cFntCtrb,@aLeiTrp,lRecIrrf )
				cString	+= valores( aISSQN, aRetido, aTotal, aDest, if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ),aLeiTrp,lRecIrrf )
				cString	+= faturas( aDupl )
				cString	+= pagtos( aDupl,cCondPag )
				cString	+= deducoes( aProd, aDeduz, aDeducao )
				cString	+= infCompl( cMensCli, cMensFis, lNFeDesc, cDescrNFSe,_lExcFisc)
	...
	Else 
		...
		cString := u_NFseM102( if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ), cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela, {} )[1]
...
/*/
//-----------------------------------------------------------------------
static function assina( aDeduz, aNota, aProd, aTotal, aDest, aDeducao )
	...
	if( len( aDeduz ) > 0 )
		...
		//-----------------------------------------------------------------------------------
		if( alltrim( if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) ) $ Fisa022Cod( "001" ) )
	...
	cAssinatura	+= strZero( val( if( lHasSigaMatX,SM0->M0_INSCM,oSigamatX:M0_INSCM ) ), 11 ) 
	cAssinatura	+= "NF   "
...
static function ident( aNota, aProd, aTotal, aDest, aISSQN, aAIDF, dDateCom )
	...
	If UsaAidfRps(if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ))
		cString	+= "<serierps>" + allTrim( aAIDF[2] ) + "</serierps>"
		cString	+= "<numerorps>" + allTrim( aAIDF[3]) + "</numerorps>"
	ElseIf (if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) $ "3303401") // Tratamento específico para Nova Friburgo - RJ
	...
	//-----------------------------------------------------------------------------------------------
	If (if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) $ "3205309-3170701")
...
static function substit( aNota )
	...
	if !empty( allTrim( aNota[8] ) + allTrim( aNota[7] ) )
		...
		cString	+= "<idnfse>" + allTrim(aNota[8]) + allTrim(aNota[7]) + "</idnfse>"
		cString	+= "<tipo>1</tipo>"
...
//			cMunPSIAFI	:= alltrim(CC2->CC2_CDSIAF) 
			cMunPSIAFI	:= '7745'  // add por alyson
	...
	If (Type( cPaisPres) <> 'U')
		If (Type( cPaisPres) <> 'C')
			If (!Empty( alltrim( cPaisPres ))) //Se o campo não estiver preenchido, a tag é preenchida com o paíss do tomador
				If (len(cPaisPres) < 4) // Se estiver preenchido com a consulta padrão da tabela de paises, haverão 3 dígitos, então a função Posicione() pega o código e faz uma busca na tabela SYA retornando o código bacem do país.
					cPaisPres := Posicione("SYA",1,xFilial("SYA")+SC5->C5_PAIS,"YA_SISEXP")
...
Static Function tomador( aDest )
	...
	If !lSemTomador 		
		...
		If if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) $ '3550308-3170701'			
		...
		If if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) $ "5208707" .And. !empty( allTrim( aDest[25] ) )
			cCodTom := aDest[25] // SA1->A1_OUTRMUN
		Else
			cCodTom := aDest[07] // SA1->A1_COD_MUN
		EndIf
		If Len( cCodTom ) <= 5 .And. (!(cCodTom $ '99999').Or. (cCodTom $ '99999' .And. if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) $ '3550308|3552205|4205407|3300704|3156700|4115200|4208203|4204202'))
			cCodTom := UfIBGEUni(aDest[09]) + cCodTom
			//customização TCP cliente exterior
			cCodTom := "7745"
		EndIf
		
		if !empty( allTrim( aDest[7] ) )
			// se for extrangeiro entra no if e coloca o codigo PSIAFI do tomador igual a 7745 paranagua  // #add alyson inicio
			IF allTrim( aDest[7] ) ==  "99999"
				cString	+= "<codmunibge>" + "7745" + "</codmunibge>"
			Else // do contrario o codigo PSIAFI normal do tombador que existe no seu cadastro SA1
				cString	+= "<codmunibge>" + allTrim( cMunPSIAFI) + "</codmunibge>" 
			Endif // #add alyson fim
		endif
		if !empty( allTrim( cMunPSIAFI) )
			cString	+= "<codmunsiafi>" + allTrim( cMunPSIAFI) + "</codmunsiafi>"
		endif
		...
		If if( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) == "3550308" .And. Len(aDest) > 21
...
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp,lRecIrrf)	
	...
	Local cMv_NFSTREC 	:= allTrim( superGetMV( "MV_NFSTREC",.F.,"" ) )
	Local cMv_CMPUSR 	:= GetMV("MV_CMPUSR")
	Default nDescon		:= 0
	Default cFntCtrb	:= ""
	Default aLeiTrp		:= {}
	Default cTpPessoa	:= ""
	...
	for nX := 1 to len( aProd )
		...
		If ( SC6->(FieldPos("C6_DESCRI")) > 0 .And. Len(aProd[nX]) > 40 .And. !Empty(aProd[nX][41]) ) .And. (!lNFeDesc .And. !GetNewPar("MV_NFESERV","1") == "1" .And. !Empty(cMv_CMPUSR) )
		...
		//-----------------------------------------------------------------------------------------
		if( cCodMun == "3550308" .and. allTrim( aProd[ nX ][ 23 ] ) $ cMv_NFSTREC )
...
/*/
//-----------------------------------------------------------------------
static function infCompl( cMensCli, cMensFis, lNFeDesc, cDescrNFSe,_lExcFisc )
	
	local cString	:= ""
	Local _cTxtLei  := if(_lExcFisc,"Lei nº 10.637, de 30 de dezembro de 2002 - Lei nº 10.833, de 29 de dezembro de 2003 Art. 5o (10.637/2002) A contribuição para o PIS/Pasep - Cofins (Art.6º 10.833/2003) não incidirá sobre as receitas decorrentes das operações de: II - prestação de serviços para pessoa física ou jurídica residente ou domiciliada no exterior, cujo pagamento represente ingresso de divisas.",'')
	...
	cString += "<observacao>" + cMensCli + space( 1 ) + cMensFis + " "+_cTxtLei+ "</observacao>"
	cString	+= "</infcompl>"
...
static function getValTotal( nValTotPed, nSD2_TOTAL )
	...
	if( (valtype("nValTotPed") <> "U") .and. (valtype("nSD2_TOTAL") <> "U") )
		if( (valtype(nValTotPed) == "N") .and. (valtype(nSD2_TOTAL) == "N") ) 
			if( iif( lHasSigaMatX,SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) $ "2927408" )
				lValSemDesc := .T.
			endif
		endif	
	endif
