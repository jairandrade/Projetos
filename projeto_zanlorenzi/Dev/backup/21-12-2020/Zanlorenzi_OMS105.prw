...
!Nome              ! OMS105.PRW                                              !
+------------------+---------------------------------------------------------+
!Descricao         ! Tela customizada cadastro de pedidos de vendas          !
+------------------+---------------------------------------------------------+
...
User Function OMS105()
	Local cQry := ""
	LOCAL aCores  := {{ 'ZA7->ZA7_STATUS=="1"' , 'ENABLE'  },;    // Ativo
	{ 'ZA7->ZA7_STATUS=="2"' , 'DISABLE' }}    // Inativo
	Local cFiltro := ""
	...
	{OemToAnsi("Cancelar EDI"),"U_OMS100C()",0,7,0,nil} } //cANCELAR EDI
	...
	cQry += " AND ZA72.D_E_L_E_T_ = ' ' ) TAB_AUX ON (ZA7.ZA7_PEDIDO  = TAB_AUX.PEDIDO "
	...
	For ni:=1 To Len(aCols)
		If (!aCols[ni][Len(aHeader)+1]) .AND. (!Empty(aCols[ni][2]))
			...
			If !lNDLDeleta
				...
				ZA7->ZA7_QUANT  := aCols[ni][3]
				ZA7->ZA7_DATA  := aCols[ni][4]
				ZA7->ZA7_HORA  := aCols[ni][5]
				ZA7->ZA7_STATUS  := "1"
			Else
				ZA7->(dbDelete())
			EndIf
...
User Function ChamaMARK()
	U_ChMARK("SC9", {"C9_PEDIDO","C9_ITEM","C9_CLIENTE","C9_LOJA","C9_PRODUTO","C9_QTDLIB"}, "", TamSx3("C9_PEDIDO")[1], "C9_PEDIDO", .T., ";")
Return
...
Public  __cRetorn := Space(nTamanM)
	...
	cFiltro := " AND EXISTS( SELECT * FROM  "+RetSQLName("SC5")+"  SC5 "
	cFiltro += " WHERE C5_FILIAL = C9_FILIAL AND C5_NUM = C9_PEDIDO AND C5_TRANSP='"+cCodTrans+"' AND SC5.D_E_L_E_T_ ='') "
	cFiltro += " AND NOT EXISTS(SELECT * FROM "+RetSQLName("ZA7")+"  ZA7 "
	//WHERE ZA7_CODIGO = '"+cNDLNum+"' "
	cFiltro += " WHERE ZA7_PEDIDO =C9_PEDIDO  AND ZA7.D_E_L_E_T_ ='')
	...
	Do While !Eof()
		If(cAliasTmp)->XX_OK =="OK"
			If cUltItem ==aCols[1][1]
				...
				//	aCols[1][3] :=(cAliasTmp)->C5_TRANSP//transportadora
				aCols[1][3] := (cAliasTmp)->C9_QTDLIB//quantidade
				//	aCols[1][5] := "000035"//codigo EDI
				aCols[1][4] := DATE()
				aCols[1][5] := TIME()
				aCols[1][6] := "ZA7"
				aCols[1][7] := 0
			...
			Else
				aAdd(aCols, {cUltItem,(cAliasTmp)->C9_PEDIDO,(cAliasTmp)->C9_QTDLIB,date(),time(),"ZA7",0, .F.})
				cUltItem :=Soma1(cUltItem)
			EndIf
		EndIf
		dbSkip()
...
User Function OMS105L()
	...
	AAdd(aLeg,{"BR_VERMELHO" , "Envio de Cancelamento transportadora"} )
