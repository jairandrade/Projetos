...
Static Function GeraArquivo(cOpc)
	...
	If !ExistDir(targetDir+"\"+cCodTransp)
		MakeDir(targetDir+"\"+cCodTransp)
	Endif

	If (nHdlArq := FCreate(targetDir+"\"+cCodTransp+"\"+cArqCPag,0)) == -1
...
//grava o ultimo codigo de registro
	...
	Else
		...
		EndIf
		ZA6->(MsUnlock())
		Aviso("Geração de Arquivo de EDI", "Arquivo gerado: "+cArqCPag+CHR(13)+CHR(10)+"Pasta: "+targetDir+"\"+cCodTransp, {"Ok"}, 1)
		...
		U_OMS100E(targetDir,cCodTransp,cArqCPag)
	Endif
Return
...
/*/{Protheus.doc} OMS100E(targetDir,cCodTransp,cArqCPag)
...
@param cCodTransp, pasta da transportadora
@param cArqCPag, Nome do Arquivo a ser enviado para o FTP
@return return_type
/*/
User Function OMS100E(targetDir,cCodTransp,cArqCPag)
	...
	If ! File(cDir+"\"+cCodTransp+"\"+cArquivo)
		cMsg := "Arquivo C:" + cDir + "\" +cCodTransp+"\"+ cArquivo + " não existe na pasta informada!!!"
		Aviso("Erro","Arquivo C:" + cDir + "\" +cCodTransp+"\"+ cArquivo + " não existe na pasta informada!!!",{"OK"}, 3 )

	Else

		cDirAnt:= cDir+"\"+cCodTransp

		If !ExistDir(cDir)
			MakeDir(cDir)
		Endif

		cDirTransp	:= cCodTransp
