	...
	If !(cAliasTemp)->(EOF())
		Begin Transaction
			While !(cAliasTemp)->(EOF())
				...
				Else
					lRet := .F.
					cMsgSC9 := "O campo C9_XSTAWMS deve estar preenchido com O= Retorno WMS OK "
						Aviso("Exclusão de N.Fiscal", "Não é possivel estornar a Nf. "+cNumDOC+" EDI: "+cCodZA7+". Motivo: "+cMsgSC9, {"Ok"}, 1)
				EndIf
				(cAliasTemp)->(dbSKip())
			Enddo
			If lRet
