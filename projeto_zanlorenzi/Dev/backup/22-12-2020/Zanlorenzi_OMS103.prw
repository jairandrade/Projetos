		...
		If aIDTrans[nX,05] == "1"
			...
			If oTree:TreeSeek( aPastaM[nPos,02] )
				...
				If oTree:TreeSeek( aPastaM[nPos,02] ) 																			//Posiciona na Pasta do Movimento
					...
					nPos := aScan( aData, { |x| x[1] == dtos(aIDTrans[nX,02])+Alltrim(Str(aIDTrans[nX,09]))+"M" } ) 							//Verifica se ja existe a Data no DbTree
					If nPos <= 0
						aAdd(aData, {dtos(aIDTrans[nX,02]), cId} )									//Adiciona a Data
						oTree:AddItem( dtoc(aIDTrans[nX,02]) , cId, "FOLDER5" ,"FOLDER6",,,3) 	//Adiciona a Data no TreeView
						If oTree:TreeSeek( cId+dtoc(aIDTrans[nX,02]) )																		//Posiciona na Pasta
							oTree:AddItem( cTexto, cId+dtoc(aIDTrans[nX,02]) , cImg,,,,4) 										//Adiciona o Texto
							oTree:PtCollApSe()
						Endif
					Else
						If oTree:TreeSeek( aData[nPos,02] )													//Encontrou a Data e Posiciona na Pasta
