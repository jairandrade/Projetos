...
//??????????????????????????????
	...
	If (ZA7->ZA7_STATUS) $'3,4,5' .and. lNDLDeleta
		HELP(' ',1,'Atenção!',,"Não é permitido excluir este cadastro ",1,0, NIL, NIL, NIL, NIL, NIL, {"Altere o STATUS do cadastro para 1-VERDE, 2=VERMELHO"})
		Return
	EndIf
//??????????????????????????????
...
User Function ChamaMARK()
	U_ChMARK("SC9", {"C9_PEDIDO","C9_ITEM","C9_CLIENTE","C9_LOJA","C9_PRODUTO","C6_DESCRI","C9_QTDLIB","C5_PESOL","C5_PBRUTO"}, "", TamSx3("C9_PEDIDO")[1], "C9_PEDIDO", .T., ";")
Return
...
Public  __cRetorn := Space(nTamanM)
	...
	cJoinSC5 += " AND C5_TRANSP='"+cCodTrans+"' AND SC5.D_E_L_E_T_ ='' " 
...
User Function OMS105L()
	...
	AAdd(aLeg,{"BR_VERMELHO" , "Pedidos cancelados"} )
