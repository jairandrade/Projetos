...
User Function OMS106F()
	...
	//Verifica a ultima Nf utilizada na SX5
	cQuery := " SELECT X5_DESCRI "
	cQuery += " FROM " + RetSqlName("SX5") + " "
	cQuery += " WHERE "
	cQuery += " X5_FILIAL = '" + xFilial("SC5") + "' "
	...
	TMP->(DbGotop())
	If !TMP->(Eof())
		AADD(_aNotas , Alltrim(TMP->X5_DESCRI))
	EndIf
	...
	Processa({ || GeraNF(_aNotas) },"Aguarde!","Gerando N.Fiscal ")
...
Static Function GeraNF(_aNotas)
	Local lContinua := .T.
	Local i         := 0
	Local cSerie	:= "80"//GetNewPar("BS_XSERGER","1")
	Local cNFSaida 	:= ""
	Local cTranspZA7:= ""
	Local cCpfZa7 := ""
	Local aPvlNfs   := {}
	...
	For i := 1 to Len(_aNotas)
		IncProc("Gerando Nota Fiscal " + _aNotas[i])
		lContinua := .T.
		_cNotaGer := _aNotas[i]
		...
		If lContinua
			...
			cQuery := " SELECT C5_CONDPAG, C5_TRANSP, ZA7_PEDIDO, ZA7_ITEMPD  "
			...
			If !TMPSC5->(Eof())
				While !TMPSC5->(Eof())
					...
					If aScan(aPedidos , TMPSC5->ZA7_PEDIDO ) <= 0
						AADD(aPedidos , TMPSC5->ZA7_PEDIDO)
					EndIf
				...
				EndDo
			Else
				cMsgErro += "Não foi possível gerar a Nota fiscal numero " + _cNotaGer +". Verifique se os pedidos estao liberados."
				lContinua := .F.
			EndIf
				...
				Begin Transaction
					...
					Else
						...
						If ZA6->(dbSeek(xFilial("ZA6")+cCodZA7))
							RecLock("ZA6",.F.)
							ZA6_MSG := ZA6_MSG+"Nf. "+cNFSaida+" foi gerada com sucesso. "+dtoc(date())+" / "+time()+CHR(13)+CHR(10)
							ZA6->(MsUnlock())
						EndIf
						//Gera Romaneio
						cMsgErro := U_GeraRoma(cTranspZA7,cCpfZa7,cNFSaida)
						//Grava na tabela de log os dados
						dbSelectArea("ZA6")
						ZA6->(dbSetOrder(1))//ZA6_FILIAL+ZA6_CODIGO+ZA6_TIPO
						If ZA6->(dbSeek(xFilial("ZA6")+cCodZA7))
						...
						EndIf
						cMsgErro :=""
						Aviso("Atenção" , "Nf. "+cNFSaida+" foi gerada com sucesso.")
					EndIf
				End Transaction
			EndIf
		endIf
			...
			If lContinua
				...
				Begin Transaction
					...
					Else
						...
						ZA7->(DbGotop())
						ZA7->(DbSetOrder(1))
						If ZA7->(DbSeek( xFilial("ZA7") + _aNotasA[i] ))
							cTranspZA7:= ZA7->ZA7_TRANSP
							cCpfZa7 := ZA7->ZA7_CPF
							While ZA7->(!Eof()) .AND. ZA7->ZA7_CODIGO == _aNotasA[i]
								ZA7->(RecLock("ZA7" , .F.))
								ZA7->ZA7_STATUS := "6"
								ZA7->ZA7_DTFAT := DATE()
								ZA7->ZA7_HRFAT := TIME()
								//ZA7->ZA7_DOC := cNFSaida
								ZA7->(MsUnLock())
								ZA7->(DbSkip())
							Enddo
							//Grava na tabela de log os dados
							dbSelectArea("ZA6")
							ZA6->(dbSetOrder(1))//ZA6_FILIAL+ZA6_CODIGO+ZA6_TIPO
							If ZA6->(dbSeek(xFilial("ZA6")+_aNotasA[i]))
								RecLock("ZA6",.F.)
								ZA6_MSG := ZA6_MSG+"Nf. "+cNFSaida+" foi gerada com sucesso. "+dtoc(date())+" / "+time()+CHR(13)+CHR(10)
								ZA6->(MsUnlock())
							EndIf
...
Return()
//----------------------------------------------------------------------------
/*/{Protheus.doc} GeraRoma(cTranspZA7,cCpfZa7,cNFSaida)
...
/*/
User Function GeraRoma(cTranspZA7,cCpfZa7,cNFSaida)
	Local oModel    := Nil
	Local cMsgErro := ""
	Local aCabec := {}
	Local cNumRom := GetSXENum("GWN", "GWN_NRROM")
...
// informe oS dados para serem carregados na tabela GWN
	...
	Begin Transaction
		...
		If lMsErroAuto
			...
			cMsgErro := MostraErro()
		Else
			cMsgErro := "Romaneio "+cNumRom+" gerado com sucesso"
			//Verifica se o documento existe na tabela GW1 e grava o codigo do romaneio GW1_NRROM
			DbSelectArea("GW1")
			GW1->(DbSetOrder(8))
			GW1->(DbGotop())
			If GW1->(DbSeek( xFilial("GW1") + cNFSaida ))
				GW1->(RecLock("GW1" , .F.))
				GW1->GW1_NRROM := cNumRom
				GW1->(MsUnLock())

				//Muda o STATUS do ROMANEIO para "Encerrado" GWN_SIT=='4'
				DbSelectArea("GWN")
				GWN->(DbSetOrder(1))
				GWN->(DbGotop())
