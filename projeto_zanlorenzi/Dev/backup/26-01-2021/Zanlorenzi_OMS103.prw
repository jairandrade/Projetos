...
//----------------------------------------------------------------------------
/*/{Protheus.doc} OMS103
LOG Integração Transportadoras X Protheus
@type function
@version 
@author Jair Andrade
@since 11/12/2020
...
Static Function ZA6Log
	Local aArea		:= GetArea()
	Local oDlg
	Private aPastaA	:= { 	{"Transportadoras"			, "1",""},;
		{"Recebimento TXT"	, "2","1"}}

	Private aPastaM	:= { 	{"Protheus"					, "3",""},;
		{"Geração TXT"		, "4","1"}}
	//	{"Envio TXT"		, "5","1"}}
	...
	oBrw:AddColumn( TcColumn():New( "Cod.Transp."		, { || aTransa[oBrw:nAt,07] }, "@!"		,,, "LEFT"		, 030, .F., .F.,,,, .F., ) )
...
Static Function fGrGrid
	...
	Endif
	cQry +=	" D_E_L_E_T_=' ' "
	cQry +=	" ORDER BY ZA6_DATA,ZA6_HRTRA,ZA6_CODIGO "
	...
	For nX:=1 to len(aIDTrans)
		...
		If aIDTrans[nX,05] == "2"
			...
			If oTree:TreeSeek( aPastaA[nPos,02] )
				...
				If nPos <= 0
					aAdd(aData, {dtos(aIDTrans[nX,02]), cId} )
					aAdd(aData, {dtos(aIDTrans[nX,02]), cId} )									//Adiciona a Data
		...
		If aIDTrans[nX,05] == "1"
			...
			If oTree:TreeSeek( aPastaM[nPos,02] )
				...
				nPos := aScan( aPastaM, { |x| x[2] == aIDTrans[nX,06] } ) 											//Procura a Pasta do Movimento
				If oTree:TreeSeek( aPastaM[nPos,02] ) 																			//Posiciona na Pasta do Movimento

					nPos := aScan( aData, { |x| x[1] == dtos(aIDTrans[nX,02]) } ) //Verifica se ja existe a Data no DbTree
					If nPos <= 0
						aAdd(aData, {dtos(aIDTrans[nX,02]), cId} )									//Adiciona a Data
						oTree:AddItem( dtoc(aIDTrans[nX,02]) , cId, "FOLDER5" ,"FOLDER6",,,2) 	//Adiciona a Data no TreeView
					Endif
...
Static Function fGrHist(pLinha)
	...
	For nX:=1 to len(aIDTrans)
		...
		If dtoc(aIDTrans[nX,02]) == alltrim(pLinha)
			aAdd( aTransa , {		Iif(aIDTrans[nX,01]=="1",oLed1,oLed2),;
				aIDTrans[nX,02],;
				aIDTrans[nX,03],;
				aIDTrans[nX,04],;
				Iif(aIDTrans[nX,05]=="1","Envio","Retorno"),;
				Iif(aIDTrans[nX,06]=="2","Recebimento TXT",Iif(aIDTrans[nX,06]=="4","Geração TXT","Envio TXT")),;
				aIDTrans[nX,07],;
				aIDTrans[nX,08],;
