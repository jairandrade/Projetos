...
User Function IMPORT03()
	...
	oProcess := tNewProcess():New("IMPORT03","Importação de Clientes",bProcess,"Rotina para importação de Clientes especifica para o MADERO. Na opção parametros, favor informar o arquivo .CSV para importação",cPerg,,.F.,,,.T.,.T.)
	...
	Local aErros := {}
		...
		lMsErroAuto := .F.
			If !lErro
				...
				Else
					MSExecAuto({|x,y| mata030(x,y)},AMATA030,3) // Inclusão
				Endif
			EndIf
