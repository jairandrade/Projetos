...
User Function JOB002( aDados, cEmpDes, cFilDes, cThreadId, nOpc )
	...
	Begin Transaction
		...
		cCodFor	:= aInclui[Ascan(aInclui,{|x| AllTrim(x[1])=="A2_COD"  }),2]
		cLojaFor:= aInclui[Ascan(aInclui,{|x| AllTrim(x[1])=="A2_LOJA" }),2]
		...
		If nOpc == 4		//Altera
			MSExecAuto({|x,y| mata020(x,y)},aAltera,nOpc)
		ElseIf nOpc == 3	//Inclui
			MSExecAuto({|x,y| mata020(x,y)},aInclui,nOpc)
		ElseIf nOpc == 5	//Exclui
			//Caso o registro não exista na outra empresa não precisa tentar excluir.
			If SA2->(DbSeek(xFilial("SA2") + cCodFor + cLojaFor))
				MSExecAuto({|x,y| mata020(x,y)},aExclui,nOpc)
			Else
				lMsErroAuto := .F.
			EndIf
		EndIf
