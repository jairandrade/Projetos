...
oReport := TReport():New("COMR001", "Relatório de Notas Entrada", cPerg, {|oReport| ReportPrint(oReport)}, "Relatório de Notas de Entrada")
...
TRCell():New( oSessao, "E2_VENCTO" 	, "", "Vencimento" 	, PesqPict("SE2","E2_VENCTO"), TamSx3("E2_VENCTO")[1])
TRCell():New( oSessao, "F4_TEXTO" 	, "", "Tipo TES" 	, PesqPict("SF4","F4_TEXTO"), TamSx3("F4_TEXTO")[1])
...
BeginSQL alias cAl
	...
	SF1.F1_DTDIGIT as D1_DTDIGIT,SF1.F1_EMISSAO,SF1.F1_DTLANC,SF1.F1_USERLGI,SF1.F1_XUSUALT,F1_XDTALT,
	SB1.B1_COD,SB1.B1_DESC ,SA2.A2_NOME AS A2_NREDUZ,E2_VENCTO,F4_TEXTO
	FROM  %table:SF1% SF1
	...
	LEFT JOIN %table:SF4% SF4 ON SF4.F4_FILIAL = SF1.F1_FILIAL  AND SF4.F4_CODIGO = SD1.D1_TES AND SF4.D_E_L_E_T_ <> '*'
	LEFT JOIN %table:SE2% SE2 ON  SE2.E2_FILIAL = SF1.F1_FILIAL AND SE2.E2_FORNECE = SF1.F1_FORNECE AND SE2.E2_LOJA = SF1.F1_LOJA
	AND SE2.E2_PREFIXO = SF1.F1_PREFIXO AND SE2.E2_NUM = SF1.F1_DOC AND SE2.D_E_L_E_T_ <> '*'
...
EndSQL
//Memowrite("c:\temp\COMR001.TXT",getLastQuery()[2])
oSessao:EndQuery()
...
Do While (!(cAl)->(Eof()))
	...
	cUserLGI := Embaralha((cAl)->F1_USERLGI, 1 )
	dDtInc 	 := CTOD("01/01/96","DDMMYY") + Load2In4(Substr(cUserLGI,16))
	
	dDtAlt :=DTOC((cAl)->F1_XDTALT)
	...
	oSessao:Cell("cUsuAlt"):SetValue((cAl)->F1_XUSUALT )
	oSessao:Cell("dUsuAlt"):SetValue(dDtAlt)
	oSessao:Cell("B1_COD"):SetValue((cAl)->B1_COD )
	oSessao:Cell("B1_DESC"):SetValue((cAl)->B1_DESC )
	oSessao:Cell("E2_VENCTO"):SetValue((cAl)->E2_VENCTO )
	oSessao:Cell("F4_TEXTO"):SetValue((cAl)->F4_TEXTO )
